--[[
	Script Name	: SpawnScripts/Castleview/planter_4.lua
	Script Purpose	: planter_4
...
function spawn(NPC)
	SetRequiredQuest(NPC, HIGH_ELF_MENTOR_QUEST_2, 4)
end
...
function casted_on(Target, Caster, SpellName)
	if HasQuest(Caster, HIGH_ELF_MENTOR_QUEST_2) and not QuestStepIsComplete(Caster, HIGH_ELF_MENTOR_QUEST_2, 4) and SpellName == "Plant Seeds" then
...
function PlantSeeds(NPC, Spawn)
	SetStepComplete(Spawn, HIGH_ELF_MENTOR_QUEST_2, 4)
	local GreenWisp = SpawnMob(GetZone(Spawn), 2360166, false, 742.82, -14.4122, -165.919, 229.984)
