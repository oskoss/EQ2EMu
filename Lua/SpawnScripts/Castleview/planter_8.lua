--[[
	Script Name	: SpawnScripts/Castleview/planter_8.lua
	Script Purpose	: planter_8
...
function spawn(NPC)
	SetRequiredQuest(NPC, HIGH_ELF_MENTOR_QUEST_2, 8)
end
...
function casted_on(Target, Caster, SpellName)
	if HasQuest(Caster, HIGH_ELF_MENTOR_QUEST_2) and not QuestStepIsComplete(Caster, HIGH_ELF_MENTOR_QUEST_2, 8) and SpellName == "Plant Seeds" then
...
function PlantSeeds(NPC, Spawn)
	SetStepComplete(Spawn,HIGH_ELF_MENTOR_QUEST_2, 8)
	local GreenWisp = SpawnMob(GetZone(Spawn), 2360171, false, 738.12, -12.9465, -115.119, 300)
